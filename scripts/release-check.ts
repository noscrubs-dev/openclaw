...
type PackedCliCommand = { argv: string[]; cwd: string };

const packageJsonPath = resolve("package.json");
const rootPackageJson = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
  name?: string;
};
const publishedPackageName = rootPackageJson.name ?? "openclaw";
...
function resolveInstalledPackageRoot(prefixDir: string, cwd: string): string {
  return join(resolveGlobalRoot(prefixDir, cwd), ...publishedPackageName.split("/"));
}

function resolveInstalledCliCommand(prefixDir: string, packageRoot: string): PackedCliCommand {
  const binaryPath =
    process.platform === "win32" ? join(prefixDir, "openclaw.cmd") : join(prefixDir, "bin", "openclaw");
  if (existsSync(binaryPath)) {
    return { argv: [binaryPath], cwd: packageRoot };
  }
  return {
    argv: [process.execPath, join(packageRoot, "openclaw.mjs")],
    cwd: packageRoot,
  };
}
...
function runPackedCliSmoke(params: {
  ...
  const command = resolveInstalledCliCommand(params.prefixDir, params.packageRoot);
  const env = createPackedCliSmokeEnv(process.env, {
    HOME: params.homeDir,
  ...
  for (const args of PACKED_CLI_SMOKE_COMMANDS) {
    if (process.platform === "win32") {
      execFileSync(
        trustedCmdPath,
        ["/d", "/s", "/c", buildCmdExeCommandLine(command.argv[0], [...command.argv.slice(1), ...args])],
        {
          cwd: command.cwd,
          stdio: "inherit",
          env,
    ...
    execFileSync(command.argv[0], [...command.argv.slice(1), ...args], {
      cwd: command.cwd,
      stdio: "inherit",
      env,
      shell: false,
...
async function runPackedBundledChannelEntrySmoke(): Promise<void> {
  ...
  try {
    ...
    const packageRoot = resolveInstalledPackageRoot(prefixDir, tmpRoot);
    ...
    await runInstalledWorkspaceBootstrapSmoke({ packageRoot });
  } finally {
