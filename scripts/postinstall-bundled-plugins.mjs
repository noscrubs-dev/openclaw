...
function ensureInstalledPackageSelfReferenceAlias(params = {}) {
  const packageRoot = params.packageRoot ?? DEFAULT_PACKAGE_ROOT;
  const pathExists = params.existsSync ?? existsSync;
  const pathLstat = params.lstatSync ?? lstatSync;
  const readFile = params.readFileSync ?? readFileSync;
  const removePath = params.rmSync ?? rmSync;
  const createDirectory = params.mkdirSync ?? mkdirSync;
  const createSymlink = params.symlinkSync ?? symlinkSync;
  const readSymlink = params.readlinkSync ?? readlinkSync;
  ...
  const aliasPath = join(packageRoot, "node_modules", "openclaw");
  const aliasTarget = relative(dirname(aliasPath), packageRoot) || ".";

  createDirectory(dirname(aliasPath), { recursive: true });
  try {
    const existing = pathLstat(aliasPath);
    if (existing.isSymbolicLink() && readSymlink(aliasPath) === aliasTarget) {
      return;
    }
    removePath(aliasPath, { recursive: true, force: true });
  } catch (error) {
  ...
  createSymlink(aliasTarget, aliasPath, process.platform === "win32" ? "junction" : "dir");
}
...
export function runBundledPluginPostinstall(params = {}) {
  ...
  ensureInstalledPackageSelfReferenceAlias({
    packageRoot,
    existsSync: pathExists,
