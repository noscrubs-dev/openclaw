...
- Fork invariant: preserve the CLI backend runtime env injection for wrapper-based backends. Child-process launches for CLI backends must continue to pass `OPENCLAW_SESSION_KEY`, `OPENCLAW_AGENT_ID`, `OPENCLAW_WORKSPACE_DIR`, `OPENCLAW_SESSION_ID`, `OPENCLAW_RUN_ID`, `OPENCLAW_PROVIDER`, and `OPENCLAW_MODEL` when available. If the CLI runner is refactored or split, keep this behavior in the active spawn path, keep the docs in `docs/gateway/cli-backends.md` accurate, and keep a regression test covering the injected env vars.
